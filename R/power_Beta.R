...
#' @usage power_Beta(n1 = NULL, n2 = NULL,  mu1 = NULL, mu2 = NULL, 
...
#' @export
power_Beta <- function(n1 = NULL, n2 = NULL,  mu1 = NULL, mu2 = NULL, 
                       sd1 = NULL, sd2 = NULL, sig.level = 0.05, power = NULL, 
                       equal.sample = TRUE, equal.precision = TRUE, trials = 100, 
                       link.type = c("logit", "probit", "cloglog", "cauchit", "log", "loglog")) {
  # Generate warnings
  ...
  ## Calculate the parameters of beta distribution
  ...
  if (equal.sample) {
    ...
    betapwr_one_samp <- function(sampsize) {
      betapwr.base <- function() {
        ...
        ### transformations, applicable when the outcomes are close to 0 or 1
        if (max(sim[, 3]) > (1 - 1e-16) | min(sim[, 3]) < 1e-16) {
          sim[, 3] <- (sim[, 3] * (sampsize * 2 - 1) + 0.5) / (sampsize * 2)
        }
        ### fit the beta regression models and calculate p-values
      ...
      ## repeat the power calculations when simulation failed with 0 or 1 aggregations.
      Power <- tryCatch(betapwr.base(), error = function(e) {
        return(NA)
      })
      while (is.na(Power[1])) {
    ...
    if (is.null(power)) {
      ...
      if (equal.precision == TRUE) {
        ...
        return(structure(list(
          N = n1, mu1 = mu1, mu2 = mu2, sd1 = sd1, sig.level = sig.level, power = Power,
        ...
        return(structure(list(
          N = n1, mu1 = mu1, mu2 = mu2, sd1 = sd1, sd2 = sd2, sig.level = sig.level, power = Power,
          method = METHOD, note = NOTE
        ),
        class = "power.htest"
        ))
    ...
    ## Calculate sample size
      ...
      if (equal.precision == TRUE) {
        METHOD <- paste0("Two-sample Beta Means Tests (Equal Sizes) (", link.type, " link, equal precision)")
        return(structure(list(
          N = sample.size.min, mu1 = mu1, mu2 = mu2, sd1 = sd1, sig.level = sig.level, power = power.output,
          method = METHOD, note = NOTE
        ),
        class = "power.htest"
        ))
        ...
        METHOD <- paste0("Two-sample Beta Means Tests (Equal Sizes) (", link.type, " link, unequal precision)")
        return(structure(list(
          N = sample.size.min, mu1 = mu1, mu2 = mu2, sd1 = sd1, sd2 = sd2, sig.level = sig.level, power = power.output,
          method = METHOD, note = NOTE
        ),
        class = "power.htest"
        ))
  ...
  # Algorithm of unequal sample situation
  else {
    if (sum(sapply(list(n1, n2, power), is.null)) != 1) {
      stop("exactly one of n1, n2 and power must be NULL")
    }
    ## define the basic power calculating function for unequal sample case
    ...
    if (is.null(power)) {
      ...
      if (equal.precision == TRUE) {
        ...
        return(structure(list(
          n1 = n1, n2 = n2, mu1 = mu1, mu2 = mu2, sd1 = sd1, sig.level = sig.level, power = Power,
          method = METHOD
        ),
        class = "power.htest"
        ))
        ...
        return(structure(list(
          n1 = n1, n2 = n2, mu1 = mu1, mu2 = mu2, sd1 = sd1, sd2 = sd2, sig.level = sig.level, power = Power,
          method = METHOD
        ),
        class = "power.htest"
        ))
    ...
    ## Calculate sample size
      ...
      if (is.null(n1)) {
        ...
        if (equal.precision == TRUE) {
          METHOD <- paste0("Two-sample Beta Means Tests (Different Sizes) (", link.type, " link, equal precision)")
          return(structure(list(
            n1 = sample.size.min, n2 = n2, mu1 = mu1, mu2 = mu2, sd1 = sd1, sig.level = sig.level, power = power.output,
            method = METHOD
          ),
          class = "power.htest"
          ))
          ...
          METHOD <- paste0("Two-sample Beta Means Tests (Different Sizes) (", link.type, " link, unequal precision)")
          return(structure(list(
            n1 = sample.size.min, n2 = n2, mu1 = mu1, mu2 = mu2, sd1 = sd1, sd2 = sd2, sig.level = sig.level, power = power.output,
            method = METHOD
          ),
          class = "power.htest"
          ))
      ...
      if (is.null(n2)) {
        ...
        if (equal.precision == TRUE) {
          METHOD <- paste0("Two-sample Beta Means Tests (Different Sizes) (", link.type, " link, equal precision)")
          return(structure(list(
            n1 = n1, n2 = sample.size.min, mu1 = mu1, mu2 = mu2, sd1 = sd1, sig.level = sig.level, power = power.output,
            method = METHOD
          ),
          class = "power.htest"
          ))
          ...
          METHOD <- paste0("Two-sample Beta Means Tests (Different Sizes) (", link.type, " link, unequal precision)")
          return(structure(list(
            n1 = n1, n2 = sample.size.min, mu1 = mu1, mu2 = mu2, sd1 = sd1, sd2 = sd2, sig.level = sig.level, power = power.output,
            method = METHOD
          ),
          class = "power.htest"
          ))
